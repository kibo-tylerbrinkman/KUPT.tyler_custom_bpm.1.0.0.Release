...
class Fulfillment_Transfer_Process_Test : JbpmJUnitBaseTestCase(true, false) {
    private var kieSession: KieSession? = null
    private var taskService: TaskService? = null

    @Before
    fun init() {
        createRuntimeManager("com/kibocommerce/bpm/fulfillment/FulfillmentProcess-Transfer.bpmn")
    ...
    private fun createProcess(): WorkflowProcessInstance {
        ...
        val processInstance = kieSession!!.startProcess("fulfillment.FulfillmentProcess-Transfer", processParam)
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(Fulfillment_Transfer_Process_Test::class.java)
    }
}