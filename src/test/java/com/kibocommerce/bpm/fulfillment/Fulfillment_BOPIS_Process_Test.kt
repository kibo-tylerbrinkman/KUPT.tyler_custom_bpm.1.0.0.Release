...
class Fulfillment_BOPIS_Process_Test : JbpmJUnitBaseTestCase(true, false) {
    private var kieSession: KieSession? = null
    private var taskService: TaskService? = null

    @Before
    fun init() {
        createRuntimeManager("com/kibocommerce/bpm/fulfillment/FulfillmentProcess-BOPIS.bpmn")
    ...
    private fun createProcess(): WorkflowProcessInstance {
        ...
        val processInstance = kieSession!!.startProcess("fulfillment.FulfillmentProcess-BOPIS", processParam)
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(Fulfillment_BOPIS_Process_Test::class.java)
    }
}