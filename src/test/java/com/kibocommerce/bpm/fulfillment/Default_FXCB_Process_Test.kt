...
class Default_FXCB_Process_Test : JbpmJUnitBaseTestCase(true, false) {
    private var kieSession: KieSession? = null
    private var taskService: TaskService? = null

    @Before
    fun init() {
        createRuntimeManager("com/kibocommerce/bpm/fulfillment/Default_FXCB_Process.bpmn")
    ...
    fun prepareForShipment() {
        ...
        assertNodeActive(wpi.id, kieSession, "Waiting for Documents")
        assertCurrentState(wpi, "WAITING_FOR_DOCUMENTS")
    }
    ...
    fun waitingForDocuments() {
        ...
        printPackingSlip(wpi, null)
        prepareForShipment(wpi, null)
        waitingForDocuments(wpi, null)

        assertNodeActive(wpi.id, kieSession, "Documents Ready")
        assertCurrentState(wpi, "DOCUMENTS_READY")
    }

    @Test
    fun waitingForDocuments_goBack() {
        val wpi = createProcess()
...
0
    ...
    private fun createProcess(): WorkflowProcessInstance {
        ...
        val processInstance = kieSession!!.startProcess("Default_FXCB_Process", processParam)
    ...
    private fun waitingForDocuments(wpi: WorkflowProcessInstance, back: Boolean?) {
        val expectedTaskName = "Waiting for Documents"
    ...
    private fun documentsReady(wpi: WorkflowProcessInstance, back: Boolean?) {
        val expectedTaskName = "Documents Ready"
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(Default_FXCB_Process_Test::class.java)
    }
}